...
@app.cell
def _hero_plot(T, hero_seed, mo, np, plt):
    def _draw():
        n = 32
...
@app.cell
def _asym_plot(T, cost_nl, cost_nu, mo, np, plt):
    def _draw():
        nl = cost_nl.value
        nu = cost_nu.value
...
def _arch_md(mo):
    ...
    PI-JEPA has three learned components: a **context encoder** $f_\theta$ (Fourier-enhanced
    backbone), an **EMA target encoder** $f_\xi$, and a **predictor bank**
    $\{g_{\phi_k}\}_{k=1}^K$ with one predictor per physical sub-operator. The total
    pretraining loss is

    $$\mathcal{L} =
      \underbrace{\mathcal{L}_{\text{pred}}}_{\text{latent prediction}}
      + \lambda_p \sum_{k=1}^K \underbrace{\mathcal{L}_{\text{phys}}^{(k)}}_{\text{PDE residual}}
      + \lambda_r \underbrace{\mathcal{L}_{\text{VICReg}}}_{\text{collapse prevention}}.$$

    No labeled trajectories enter the pretraining objective. The predictor bank
    structure — with one module per sub-operator — is the central architectural
    contribution, identified as responsible for a 16% performance gain in ablation.
    """)
    return
...
@app.cell
def _split_plot(T, sp_seed, sp_typ, mo, np, plt, gridspec):
    def _draw():
        n    = 32
...
@app.cell
def _de_plot(T, bench_sel, methods_sel, nl_line, mo, np, plt):
    _METHOD_STYLE = {
...
@app.cell
def _sc_plot(T, sc_K, sc_d, sc_n, mo, np, plt):
    def _draw():
        n  = int(sc_n.value)
        d  = int(sc_d.value)
        K_ = int(sc_K.value)
...
def _sigreg_controls(mo):
    ...
    return sr_col, sr_d, sr_M


@app.cell
def _sigreg_plot(T, sr_col, sr_d, sr_M, mo, np, plt):
    def _draw():
        d     = int(sr_d.value)
        M     = int(sr_M.value)
...
@app.cell
def _sigreg_widgets(mo, sr_col, sr_d, sr_M):
